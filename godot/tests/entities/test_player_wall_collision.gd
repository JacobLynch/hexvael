...
# Arena is 480x320, walls are 8px thick centered at boundaries.
# Player is 12x12 (6px half-extent). Wall inner edges:
#   Top:    y = 0     (wall center y=-4, half-thickness=4)
#   Bottom: y = 320   (wall center y=324, half-thickness=4)
#   Left:   x = 0     (wall center x=-4, half-thickness=4)
#   Right:  x = 480   (wall center x=484, half-thickness=4)
# Player stops when its edge meets the wall edge, so min/max position
# is offset by the player's 6px half-extent: x in [6, 474], y in [6, 314].
