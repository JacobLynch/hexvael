...
func _process(delta: float):
	if _net_client.is_server_connected():
		...
		if _input_provider.consume_fire_press():
			...
			# Block firing during ghost state - can't attack while dead.
			if _local_player != null and _local_player.state != PlayerMovementState.GHOST:
